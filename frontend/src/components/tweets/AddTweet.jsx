...
import { creatTweet } from "../../features/tweets/tweetSlice";

function AddTweet({ onClose }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      await dispatch(creatTweet({ content })).unwrap();
      onClose();
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-black text-white border border-gray-300 p-6 rounded-sm w-full max-w-md shadow-lg relative">
        <h2 className="text-lg font-semibold mb-4">Add Tweet</h2>
